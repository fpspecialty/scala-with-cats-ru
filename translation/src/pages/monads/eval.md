...
Тем не менее, мы должны помнить, что за трамполайн тоже приходится платить.
Он позволяет избежать потребления стека, создавая последовательность функциональных объектов в куче (heap).
Предел вложенности таких вычислений всё ещё существует,
но определяется он размером кучи, а не стека.

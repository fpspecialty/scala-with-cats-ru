...
Как это связано с общей схемой 
...
и каждый раз, когда применяется `map`, в последовательность добавляется еще одна операция.
Вызов `map` на самом деле не *запускает* ни одну из операций,
но если передать аргумент конечной функции,
...
будь то последовательность множества небольших операций, следующих одна за другой,
или их объединение в большую функцию перед применением `map`.
...
Род (kind) как тип для типов.
Они описывают количество "дырок" в типе.
Мы различаем обычные типы, у которых нет "дырок", 
и "конструкторы типов", у которых есть
"дырки", которые можно заполнить для создания типов.

Например, `List` - это конструктор типов с одной "дыркой".
Заполним эту "дырку", указав параметр для создания
...
можно увидеть, что в Cats определение `Functor`
позволяет создавать экземпляры для любого конструктора типа от одного параметра,
